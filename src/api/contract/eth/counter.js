...
const CounterContract = web3.eth.contract(counterABI).at(network);

export const inc = () => new Promise((resolve, reject) => {
    CounterContract.inc({
      value: 0,
      gas: 220000,
...
export const get = () => Promise.promisify(CounterContract.get)();
...
import cryptoWaterMarginABI from './abi/counter.json';
import convertContractABI from './abi/counter.json';
...
export const buyItem = (id, price) => new Promise((resolve, reject) => {
  cryptoWaterMarginContract.inc(id, {
    gas: 220000,
    gasPrice: 1000000000 * 100,
  },
  (err, result) => (err ? reject(err) : resolve(result)));
